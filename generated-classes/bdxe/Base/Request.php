...
/**
 * Base class that represents a row from the 'Request_tb' table.
 *
 * 
 *
...
abstract class Request implements ActiveRecordInterface 
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\bdxe\\Map\\RequestTableMap';
    ...
    /**
     * Initializes internal state of bdxe\Base\Request object.
     */
     ...
     * @return $this|\bdxe\Request The current object (for fluent API support)
     */
    ...
    public function setStudentId($v)
    {
        ...
        if ($this->student_id !== $v) {
            $this->student_id = $v;
            $this->modifiedColumns[RequestTableMap::COL_STUDENT_ID] = true;
        }
    ...
    public function setCourseId($v)
    {
        ...
        if ($this->course_id !== $v) {
            $this->course_id = $v;
            $this->modifiedColumns[RequestTableMap::COL_COURSE_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : RequestTableMap::translateFieldName('StudentId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->student_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : RequestTableMap::translateFieldName('CourseId', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\bdxe\\Request'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if (null === $this->id) {
            try {                
                $dataFetcher = $con->query('SELECT Request_tb_SEQ.nextval FROM dual');
        ...
        if ($this->isColumnModified(RequestTableMap::COL_STUDENT_ID)) {
            $modifiedColumns[':p' . $index++]  = 'student_id';
        }
        if ($this->isColumnModified(RequestTableMap::COL_COURSE_ID)) {
            $modifiedColumns[':p' . $index++]  = 'course_id';
        }

        $sql = sprintf(
            'INSERT INTO Request_tb (%s) VALUES (%s)',
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\bdxe\Request The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(RequestTableMap::COL_STUDENT_ID)) {
            $criteria->add(RequestTableMap::COL_STUDENT_ID, $this->student_id);
        }
        if ($this->isColumnModified(RequestTableMap::COL_COURSE_ID)) {
            $criteria->add(RequestTableMap::COL_COURSE_ID, $this->course_id);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \bdxe\Request (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \bdxe\Request Clone of current object.
     * @throws PropelException
     */
    ...
    public function getStudent(ConnectionInterface $con = null)
    {
        if ($this->aStudent === null && ($this->student_id !== null)) {
            ...
            /* The following can be used additionally to
                ...
                $this->aStudent->addRequests($this);
             */
        }
    ...
    {
        if (null !== $this->aStudent) {
            $this->aStudent->removeRequest($this);
        }
        if (null !== $this->aCourse) {
            $this->aCourse->removeRequest($this);
        }
        $this->id = null;
