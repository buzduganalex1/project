...
abstract class Student implements ActiveRecordInterface 
{
    ...
    /**
     * The value for the teachermessage field.
     * @var        string
     */
    ...
    /**
     * Get the [teachermessage] column value.
     * 
     * @return string
     */
    ...
    public function setRequest($v)
    {
        ...
        if ($this->request !== $v) {
            $this->request = $v;
            $this->modifiedColumns[StudentTableMap::COL_REQUEST] = true;
        }
    ...
    /**
     * Set the value of [teachermessage] column.
     * 
     * @param string $v new value
     * @return $this|\bdxe\Student The current object (for fluent API support)
     */
    public function setTeachermessage($v)
    {
        if ($v !== null) {
            $v = (string) $v;
        }

        if ($this->teachermessage !== $v) {
            $this->teachermessage = $v;
            $this->modifiedColumns[StudentTableMap::COL_TEACHERMESSAGE] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : StudentTableMap::translateFieldName('Request', TableMap::TYPE_PHPNAME, $indexType)];
            $this->request = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : StudentTableMap::translateFieldName('Teachermessage', TableMap::TYPE_PHPNAME, $indexType)];
            $this->teachermessage = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : StudentTableMap::translateFieldName('Password', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(StudentTableMap::COL_REQUEST)) {
            $modifiedColumns[':p' . $index++]  = 'Request';
        }
        if ($this->isColumnModified(StudentTableMap::COL_TEACHERMESSAGE)) {
            $modifiedColumns[':p' . $index++]  = 'TeacherMessage';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'Request':                        
                        ...
                        break;
                    case 'TeacherMessage':                        
                        $stmt->bindValue($identifier, $this->teachermessage, PDO::PARAM_STR);
                        break;
                    case 'password':                        
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[4], $arr)) {
            $this->setTeachermessage($arr[$keys[4]]);
        }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(StudentTableMap::COL_REQUEST)) {
            $criteria->add(StudentTableMap::COL_REQUEST, $this->request);
        }
        if ($this->isColumnModified(StudentTableMap::COL_TEACHERMESSAGE)) {
            $criteria->add(StudentTableMap::COL_TEACHERMESSAGE, $this->teachermessage);
        }
        if ($this->isColumnModified(StudentTableMap::COL_PASSWORD)) {
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        $copyObj->setTeachermessage($this->getTeachermessage());
