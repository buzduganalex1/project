...
/**
 * Base class that represents a row from the 'group_tb' table.
 *
 * 
 *
...
abstract class Group implements ActiveRecordInterface 
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\bdxe\\Map\\GroupTableMap';
    ...
    /**
     * Initializes internal state of bdxe\Base\Group object.
     * @see applyDefaults()
     */
     ...
     * @return $this|\bdxe\Group The current object (for fluent API support)
     */
    ...
    public function setProjectId($v)
    {
        ...
        if ($this->project_id !== $v) {
            $this->project_id = $v;
            $this->modifiedColumns[GroupTableMap::COL_PROJECT_ID] = true;
        }
    ...
    public function setNr($v)
    {
        ...
        if ($this->nr !== $v) {
            $this->nr = $v;
            $this->modifiedColumns[GroupTableMap::COL_NR] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : GroupTableMap::translateFieldName('ProjectId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->project_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : GroupTableMap::translateFieldName('Nr', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\bdxe\\Group'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if (null === $this->id) {
            try {                
                $dataFetcher = $con->query('SELECT group_tb_SEQ.nextval FROM dual');
        ...
        if ($this->isColumnModified(GroupTableMap::COL_PROJECT_ID)) {
            $modifiedColumns[':p' . $index++]  = 'project_id';
        }
        if ($this->isColumnModified(GroupTableMap::COL_NR)) {
            $modifiedColumns[':p' . $index++]  = 'nr';
        }

        $sql = sprintf(
            'INSERT INTO group_tb (%s) VALUES (%s)',
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\bdxe\Group The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(GroupTableMap::COL_PROJECT_ID)) {
            $criteria->add(GroupTableMap::COL_PROJECT_ID, $this->project_id);
        }
        if ($this->isColumnModified(GroupTableMap::COL_NR)) {
            $criteria->add(GroupTableMap::COL_NR, $this->nr);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \bdxe\Group (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \bdxe\Group Clone of current object.
     * @throws PropelException
     */
     ...
     * @return ObjectCollection|ChildPack[] List of ChildPack objects
     */
    public function getPacksJoinSubscription(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildPackQuery::create(null, $criteria);
    ...
    {
        if (null !== $this->aProject) {
            $this->aProject->removeGroup($this);
        }
        $this->id = null;
